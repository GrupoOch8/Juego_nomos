...
public class Pep {
	 ...
	 */
	public Pep(int x, int y, int ancho, int alto) {
		this.coordenadaX = x;
		this.coordenadaY = y;
		this.ancho = ancho;
		this.alto = alto;
	 ...
	 */
	public void dibujar(Entorno e, Image imgPep) {
		if (imgPep != null) {
			e.dibujarImagen(imgPep, coordenadaX, coordenadaY, 0, 0.09);
		}
	}
	...
	/**
	 * Mueve a Pep hacia la izquierda. Disminuye la coordenada X y ajusta la
	 * dirección.
	 */
	...
	/**
	 * Mueve a Pep hacia la derecha. Aumenta la coordenada X y ajusta la dirección.
	 */
	...
	/**
	 * Hace que Pep salte si no está en el aire. Ajusta la velocidad vertical para
	 * simular el salto.
	 */
	public void saltar() {
		if (!this.enElAire) {
	...
	/**
	 * Aplica gravedad a Pep si está en el aire. Aumenta la velocidad vertical y
	 * ajusta la posición en Y.
	 */
	...
	/**
	 * Verifica colisiones de Pep con las islas. Ajusta la posición de Pep si hay
	 * colisión y controla su movimiento.
	 * 
	...
	private boolean verificarColisionLateral(Isla isla, int limIzq, int limDer, int limBot, int limTop) {
		boolean colisionLadoDerecho = limDer >= isla.getX() - isla.getAncho() / 2 && limIzq < isla.getX()
				&& limBot > isla.getY() - isla.getAlto() / 2 && limTop < isla.getY() + isla.getAlto() / 2;

		boolean colisionLadoIzquierdo = limIzq <= isla.getX() + isla.getAncho() / 2 && limDer > isla.getX()
				&& limBot > isla.getY() - isla.getAlto() / 2 && limTop < isla.getY() + isla.getAlto() / 2;
	...
	/**
	 * Verifica si Pep ha llegado a los límites de la pantalla y ajusta su posición
	 * en consecuencia.
	 */
